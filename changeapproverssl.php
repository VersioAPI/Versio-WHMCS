...
$ca->setPageTitle('Change approver');

$ca->addToBreadCrumb('index.php', Lang::trans('globalsystemname'));
$ca->addToBreadCrumb('changeapproverssl.php', 'Change approver');
...
$ca->assign('url', 'changeapproverssl.php');
...
if ($ca->isLoggedIn()) {
 ...
 header('HTTP/1.0 403 Forbidden');
  ...
  $endpoint = $login[2]->value;
    ...
    $response = $versio->request('POST', '/sslcertificates/'.$order->order_id.'/changeapprover', $details);
	...
	if($response['error'])
			...
			if($response['id'])
		{
			...
			$ca->assign('message', 'Approver e-mail changed');
